...
const createCollege = async function (req, res) {
    try {
        ...
        if (!isValid(logoLink)) return res.status(400).send({ status: false, message: "Logo Link is invalid" })
...
const getCollegeDetails = async function (req, res) {
    try {
        ...
        if(!query.name.trim().length) return res.status(400).send({status:false,message:"please provied name of college"})
